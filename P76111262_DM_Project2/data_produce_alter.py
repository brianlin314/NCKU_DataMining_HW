...
for i in range(500):
    ...
    if temp[4]=="Serious" and (temp[5]==2 or temp[5]==3 or temp[5]==4 or temp[5]==5 or temp[5]==6) and temp[6]=="Physical" and temp[7]=="High" and (temp[9]=="Middle" or temp[9]=="High") and temp[10]=="Yes":
        temp.append("good")
        positive+=1
    else:
        temp.append("bad")
...
for i in range(500,1000):
    temp=[random.randint(25,65), random.choice(["M","F"]),random.randint(140,190),random.randint(40,100),
        "Serious",random.randint(2,6),"Physical",
        "High",random.randint(0,1),random.choice(["Middle","High"]),"Yes","good"]
    positive+=1
    data.loc[i]=temp
...
data_shuffled.to_csv("teacher_data_alter.csv",index=False)